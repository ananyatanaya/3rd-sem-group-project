...
while True:  
    ...
    for i in pos_dict.keys():
        if i in close_objects:
            ...
            COLOR = (0,255,0)     
        # Convert mms to feet
        print("Distance: {i} ft".format(i=round(pos_dict[i][2]/30.48,4)))
    ...
    try:
        ...
        posX = pos[0] + 5
        posY = pos[0] - 5
        print("Face Detected {}%".format(str(confidence)))
        if foundFace == True:
            print("Found user {}".format(user))
            print("Face Detected {}%".format(confidence))
        else:
            print("Unknown Face")
    ...
    # Raise exception in case, no image is found
        ...
        print("Accuracy 0% (No face detected)")
        pass
